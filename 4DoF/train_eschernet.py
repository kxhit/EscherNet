        ...
        for valid_step, batch in tqdm(enumerate(validation_dataloader)):
            ...
            for _ in range(args.num_validation_images):
                with torch.autocast("cuda"):
                    image = pipeline(input_imgs=input_image, prompt_imgs=input_image, poses=[pose_out, pose_in], height=h, width=w, T_in=T_in, T_out=pose_out.shape[1],
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet, image_encoder):
                ...
                model_pred = unet(
                    ...
                    pose=[pose_out, pose_in],                    # (bxT_in) 4, pose_out - self-attn, pose_in - cross-attn
                ).sample
